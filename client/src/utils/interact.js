...
function Interact(props) {
  ...
  const listMyRides = async (myAddress) => {
    ...
    for (var rideNumber = 0; rideNumber < count; rideNumber++) {
      ...
      if (
        (ride[0].toLowerCase() === myAddress || ride[12].includes(myAddress)) &&
        (ride[7] === "passengerRequested" ||
          ride[7] === "passengerConfirmed" ||
          ride[7] === "enroute" ||
          ride[7] === "completed")
      ) {
        // var dateTime = new Date(Number(ride[6]))
  ...
  const startRide = async (rideNumber) => {
    const ride = await contractInstance.methods
      .startRide(rideNumber)
      .send({ from: currentAccount });
    console.log(ride);
  };
  ...
  const acceptPassengerRequest = async (rideNumber, passengerAddress) => {
    const ride = await contractInstance.methods
      .acceptPassengerRequest(rideNumber, passengerAddress)
      .send({ from: currentAccount });
    console.log(ride);
  };
  ...
  const rejectPassengerRequest = async (rideNumber, passengerAddress) => {
    const ride = await contractInstance.methods
      .rejectPassengerRequest(rideNumber, passengerAddress)
      .send({ from: currentAccount });
    console.log(ride);
  };
  ...
  const getUserCount = () => {
    axios.get("http://localhost:9002/userCount").then((res) => {
  ...
  return (
    <contractContext.Provider
      value={{
        ...
        availableRides,
        otherRides,
        listMyRides: listMyRides,
        myRides,
        checkWalletIsConnected: checkWalletIsConnected,
        connectWalletHandler: connectWalletHandler,
        checkWalletConnected: checkWalletConnected,
        createRide: createRide,
        getTotalRideCount: getTotalRideCount,
        ...
        getAvailableRideCount: getAvailableRideCount,
        availableRideCount,
        getEnrouteRideCount: getEnrouteRideCount,
        getCompletedRideCount: getCompletedRideCount,
        enrouteRideCount,
        completedRideCount,
        getUserCount: getUserCount,
        rejectPassengerRequest: rejectPassengerRequest,
        userCount,
      }}
    >
      {props.children}
